...
type Config struct {
	...
	SettlementBacklogHigh     int
	SettlementBacklogLow      int
	OutboxBacklogHigh         int
	OutboxBacklogLow          int
...
func Load() (Config, error) {
	cfg := Config{
		...
		SettlementBacklogHigh:     100,
		SettlementBacklogLow:      40,
		OutboxBacklogHigh:         600,
		OutboxBacklogLow:          240,
	...
	for name, target := range map[string]*int{
		"ADFLOW_SETTLEMENT_BACKLOG_HIGH": &cfg.SettlementBacklogHigh,
		"ADFLOW_SETTLEMENT_BACKLOG_LOW":  &cfg.SettlementBacklogLow,
		"ADFLOW_OUTBOX_BACKLOG_HIGH":     &cfg.OutboxBacklogHigh,
		"ADFLOW_OUTBOX_BACKLOG_LOW":      &cfg.OutboxBacklogLow,
	} {
		if *target, err = intEnv(name, *target); err != nil {
			return Config{}, err
		}
	}
	if cfg.SettlementBacklogHigh <= 0 || cfg.SettlementBacklogLow < 0 || cfg.SettlementBacklogLow >= cfg.SettlementBacklogHigh || cfg.OutboxBacklogHigh <= 0 || cfg.OutboxBacklogLow < 0 || cfg.OutboxBacklogLow >= cfg.OutboxBacklogHigh {
		return Config{}, fmt.Errorf("backlog thresholds require 0 <= low < high")
	}
	if cfg.DemoData, err = boolEnv("ADFLOW_DEMO_DATA", true); err != nil {
		return Config{}, err
	}
	...
	if cfg.EventTransport == "kafka" && (cfg.ReservationAdapter != "redis" || cfg.DecisionStore != "mysql") {
		return Config{}, fmt.Errorf("Kafka settlement recovery requires ADFLOW_RESERVATION_ADAPTER=redis and ADFLOW_DECISION_STORE=mysql")
	}
