...
type Metrics struct {
	...
	decisionBacklog             *prometheus.GaugeVec
	decisionBackpressureState   *prometheus.GaugeVec
	decisionBackpressureResults *prometheus.CounterVec
...
func New() *Metrics {
	m := &Metrics{
		...
		decisionBacklog: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Namespace: "adflow", Subsystem: "decision", Name: "async_backlog", Help: "Sampled active queue counts, capped at high watermarks; valid only while observation is available.",
		}, []string{"queue"}),
		decisionBackpressureState: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Namespace: "adflow", Subsystem: "decision", Name: "backpressure_state", Help: "Last queue observation state (one-hot); new decisions also fail closed after 1s of staleness.",
		}, []string{"state"}),
		decisionBackpressureResults: prometheus.NewCounterVec(prometheus.CounterOpts{
			Namespace: "adflow", Subsystem: "decision", Name: "backpressure_total", Help: "New-decision backlog gate outcomes, excluding existing decision replay.",
		}, []string{"result"}),
	...
	m.registry.MustRegister(
		...
		m.decisionBacklog, m.decisionBackpressureState, m.decisionBackpressureResults,
...
func (m *Metrics) SetDecisionBacklog(settlements, outbox int64, state string) {
	m.decisionBacklog.WithLabelValues("settlements").Set(float64(settlements))
	m.decisionBacklog.WithLabelValues("outbox").Set(float64(outbox))
	for _, label := range []string{"open", "blocked", "unavailable"} {
		value := 0.0
		if state == label {
			value = 1
		}
		m.decisionBackpressureState.WithLabelValues(label).Set(value)
	}
}

func (m *Metrics) ObserveDecisionBackpressure(result string) {
	m.decisionBackpressureResults.WithLabelValues(result).Inc()
}
...
func (m *Metrics) SetOutboxDepth(stats eventdomain.OutboxStats) {
	m.outboxDepth.WithLabelValues("settling").Set(float64(stats.Settling))
	m.outboxDepth.WithLabelValues("reconcile").Set(float64(stats.Reconcile))
