...
import { api, ApiError, simulationAPIBase } from '@/lib/api';
import {
  allowDemoAutoLogin,
  demoLoginPaused,
  pauseDemoLogin,
...
export function AuthGate({ children }: { children: React.ReactNode }) {
    ...
    void api
      ...
      .catch(async (cause) => {
        ...
        if (cause instanceof ApiError && cause.status === 401) {
          const automatic = allowDemoAutoLogin(
            process.env.NEXT_PUBLIC_ADFLOW_DEMO_AUTO_LOGIN === 'true',
            window.location.href,
            simulationAPIBase,
            demoLoginPaused(),
          );
          if (!automatic) return;
          const autoAttempt = ++generation.current;
          try {
            // Public local/test seed account. Uses normal login and real JWT validation.
            const issued = await api.login('admin', 'adflow-admin');
            if (!alive || autoAttempt !== generation.current) return;
            setSession(issued);
            const info = await api.authMe();
            if (alive && autoAttempt === generation.current) setAuth(info);
          } catch {
            if (alive && autoAttempt === generation.current) {
              clearSession();
              setError('本机 admin 自动登录失败，请手动登录。');
            }
          }
        } else {
          setUnavailable(true);
  ...
  async function login(event: { preventDefault(): void }) {
    ...
    const attempt = ++generation.current;
    try {
      const issued = await api.login(username.trim(), password);
      ...
      setPassword('');
      if (
        info.principal.username === 'admin' &&
        info.principal.role === 'admin'
      )
        resumeDemoLogin();
      else pauseDemoLogin();
      setAuth(info);
      ...
      <div className="console-login-panel w-full max-w-sm overflow-hidden rounded-xl border bg-card">
        <section className="flex flex-col justify-center gap-6 p-7 sm:p-9">
          <div className="space-y-2">
            ...
            <h1 className="text-2xl font-semibold">登录 AdFlow</h1>
