...
for name,digest in data['sourceSHA256'].items():
    candidates=[pathlib.Path(name),directory/'source'/name,directory.parent/'source'/name,directory/'source'/(name+'.txt'),directory.parent/'source'/(name+'.txt')]
    if name=='tests/bench/k6-lab/kafka_capacity.go': candidates.append(directory/'tested-kafka-capacity.go.txt')
    if name=='tests/bench/k6-lab/main.go': candidates.append(directory/'tested-main.go.txt')
    assert any(p.exists() and hashlib.sha256(p.read_bytes()).hexdigest()==digest for p in candidates),name
stages=[]
incomplete=[]
...
for stage in data['capacityResults']:
    ...
    if data.get('backpressureExperiment'):
        assert stage['backpressureRejected']==v('backpressure_rejections','count')
        passed=passed and v('business_errors','rate')==0 and stage['matched']>0 and stage['matched']+stage['backpressureRejected']==stage['iterations'] and v('matched_decision_ms','p(95)')<300
    ...
    if data.get('backpressureExperiment'):
        stages[-1].update({'attempts':stage['iterations'],'expectedBackpressureRejections':stage['backpressureRejected'],'matchedDecisionP95Ms':v('matched_decision_ms','p(95)'),'unexpectedBusinessErrorRate':v('business_errors','rate'),'admittedRoundsPerConfiguredSecond':stage['completed']/stage['seconds']})
    total_loops+=stage['iterations'];total_events+=stage['acceptedEvents']
confirmed=data.get('confirmedBusinessRoundsPerSecond')
if data.get('protectionPassed'):
    assert not confirmed and not data.get('error')
    assert [s['label'] for s in data['capacityResults']]==['baseline','overload','recovery']
    assert all(s['passed'] for s in data['capacityResults'])
    assert data['capacityResults'][1]['backpressureRejected']>0
    assert data['capacityResults'][0]['backpressureRejected']==data['capacityResults'][2]['backpressureRejected']==0
...
print(json.dumps({'verified':True,'backpressureExperiment':data.get('backpressureExperiment',False),'protectionPassed':data.get('protectionPassed'),'incompleteStages':incomplete,'confirmedBusinessRoundsPerSecond':confirmed,'measuredLoopsAllStages':total_loops,'acceptedEventsAllStages':total_events,'confirmationLoops':sum(s['iterations'] for s in last),'confirmationEvents':sum(s['acceptedEvents'] for s in last),'confirmedCombinedP95Ms':lat[math.ceil(len(lat)*.95)-1] if confirmed else None,'stages':stages,'rawSHA256':hashlib.sha256(raw).hexdigest()},indent=2))
