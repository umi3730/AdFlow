...
export function handleSummary(data){const lines=['Kafka complete business workload; asynchronous proof is collected separately'];for(const n of ['iterations','http_reqs','decision_ms','accepted_round_ms','business_errors','matched_decisions','completed_rounds','accepted_events','accepted_impressions','accepted_clicks','accepted_conversions','no_ad','dropped_iterations'])if(data.metrics[n])lines.push(`${n}: ${JSON.stringify(data.metrics[n])}`);return{[__ENV.REPORT_PATH]:JSON.stringify(data,null,2),stdout:lines.join('\n')+'\n'}}
