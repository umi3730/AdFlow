...
export function handleSummary(data) {
  ...
  // k6 includes setup() output here, which contains the authenticated token.
  // Only export the measurement summary; keep credentials out of artifacts.
  const summary = { ...data };
  delete summary.setup_data;
  return { [__ENV.REPORT_PATH || 'k6-summary.json']: JSON.stringify(summary, null, 2), stdout: lines.join('\n') + '\n' };
}
